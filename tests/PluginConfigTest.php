...
class PluginConfigTest extends PHPUnit_Framework_TestCase {
	 ...
	 */
	public function test_PluginConfig($comp_category, $comp_name, $plugin_slot, $name, $git, $valid) {
		if ($valid) {
			$this->_test_valid_PluginConfig($comp_category, $comp_name, $plugin_slot, $name, $git);
		}
		else {
			$this->_test_invalid_PluginConfig($comp_category, $comp_name, $plugin_slot, $name, $git);
		}
	}

	public function _test_valid_PluginConfig($comp_category, $comp_name, $plugin_slot, $name, $git) {
		$config = new Plugin($comp_category, $comp_name, $plugin_slot, $name, $git);
	...
	public function _test_invalid_PluginConfig($comp_category, $comp_name, $plugin_slot, $name, $git) {
		try {
			$config = new Plugin($comp_category, $comp_name, $plugin_slot, $name, $git);
			$this->assertFalse("Should have raised.");
		}
				...
				foreach ($this->pluginSlotProvider() as $plugin_slot) {
					foreach ($this->nameProvider() as $name) {
						foreach ($this->gitBranchProvider() as $git) {
							$ret[] = array
								( $comp_category[0], $comp_name[0], $plugin_slot[0], $name[0], $git[0]
								, $comp_category[1] && $comp_name[1] && $plugin_slot[1] && $name[1] && $git[1]);
						}
					}
				}
			}
